...
from django.views.decorators.csrf import csrf_protect
from .models import ind_clientDT, GROUP, Client_SYMBOL_QTY
...
def client_login(request):
    ...
    if request.method == 'POST':
        ...
        try:
            user = ind_clientDT.objects.get(clint_email=email)
            if password == user.clint_password:
                user_id = user.clint_id
                if not user.clint_last_login:  # Check if the user has never logged in before
                    request.session['clint_id'] = user_id
                    request.session['new_user'] = True  # Indicate that this is the first login
                    return redirect('/terms-and-conditions')
                else:
                    request.session['clint_id'] = user_id
                    group = user.client_Group
                    alls = group.symbols.all()
                    for s in alls:
                        try:
                            signals = Client_SYMBOL_QTY.objects.get(client_id=user_id, SYMBOL=s)
                            q = signals.QUANTITY
                            d = signals.client_id
                        except:
                            q = 0
                    ...
                    user.clint_last_login = timezone.now()
                    user.save()
            ...
            print(e)
            return redirect(f'/?msg=Email not registered: {e}')
    return render(request, 'client_login.html', {'msg': msg})
...
def accept_terms(request):
    if request.method == 'POST':
        ...
        if clint_id:
            user = ind_clientDT.objects.get(clint_id=clint_id)
            subject = f'Dear {user.clint_name_first} {user.clint_name_last}'
            message = f"""Dear {user.clint_name_first} {user.clint_name_last},

All subscription fees paid to 'Anvestors.com' is Non refundable. We do not provide trading tips nor are we investment advisers. Our service is solely restricted to automated trading application development, deployment, and maintenance. All algorithms are based on backtested data but we do not provide any guarantee for their performance in the future. The algorithm running in an automated system is agreed with the user prior to deployment and we do not take any liability for any loss generated by the same. Past performance of advice/strategy/model does not indicate the future performance of any current or future strategy/model or advice by 'Anvestors.com' Trades and actual returns may differ significantly from that depicted herein due to various factors including but not limited to impact costs, expense charged, timing of entry/exit, timing of additional flows/redemptions, individual client mandates, specific portfolio construction characteristics, etc. There is no assurance or guarantee that the objectives of any strategy/model or advice provided by 'Anvestors.com' Trades will be achieved. 'Anvestors.com' Trades or any of its partners or principal officers/employees do not assure/give a guarantee for any return on the investment in strategies/models/advice given to the Investor. The value of the investment can go up/down depending on factors & forces affecting securities markets. 'Anvestors.com' Trades or its associates are not liable or responsible for any loss or shortfall arising from operations and affected by the market condition.
...
def homes(request):
    ...
    #    trades = Trade.objects.filter(clint_id=user_id).order_by('timestamp')
        ...
        while short_entries and short_exits:
            ...
            # total_cumulative_pnl += pnl
            pnl = (trades.price - entry_trade.price) * trades.lot_size if trades.trade_type == 'Long_Exit' else (entry_trade.price - trades.price) * trades.lot_size
            total_cumulative_pnl += pnl
            total_trades += 1
            if pnl > 0:
                profitable_trades += 1
